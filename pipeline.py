...
On Streamlit Cloud (no JVM), automatically falls back to pure Pandas.
...
# ── Robust data directory — works locally AND on Streamlit Cloud ──────────────
_HERE    = Path(__file__).resolve().parent          # directory of pipeline.py
DATA_DIR = _HERE / "data"                           # always correct regardless of cwd

# ── Try PySpark; graceful fallback if JVM unavailable (Streamlit Cloud) ───────
try:
