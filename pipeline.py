...
pipeline.py
===========
PySpark ETL → unified Pandas DataFrames consumed by the Streamlit app.
All heavy lifting lives here; the app just calls load_all().
"""
...
# ── Try PySpark; fall back to pure Pandas if JVM unavailable ──────────────────
try:
...
def _spark_pipeline():
    ...
    fb_raw = spark.read.csv(f"{DATA_DIR}/01_facebook_ads.csv",  header=True, inferSchema=True)
    g_raw  = spark.read.csv(f"{DATA_DIR}/02_google_ads.csv",    header=True, inferSchema=True)
    tt_raw = spark.read.csv(f"{DATA_DIR}/03_tiktok_ads.csv",    header=True, inferSchema=True)
    ...
    # ── 7-day rolling spend (Window function) ──
...
def _pandas_pipeline():
    fb = pd.read_csv(f"{DATA_DIR}/01_facebook_ads.csv", parse_dates=["date"])
    g  = pd.read_csv(f"{DATA_DIR}/02_google_ads.csv",   parse_dates=["date"])
    tt = pd.read_csv(f"{DATA_DIR}/03_tiktok_ads.csv",   parse_dates=["date"])

    fb = fb.assign(
        platform="Facebook", video_views=fb["video_views"].astype(float),
        ctr=fb["clicks"]/fb["impressions"], cpc=fb["spend"]/fb["clicks"],
        cpa=fb["spend"]/fb["conversions"],  roas=np.nan,
        social_eng=0.0, vid_compl_rate=np.nan, conv_value=np.nan,
    )
    g = g.assign(
        platform="Google", spend=g["cost"].astype(float), video_views=0.0,
        ctr=g["ctr"], cpc=g["avg_cpc"],
        cpa=g["cost"]/g["conversions"], roas=g["conversion_value"]/g["cost"],
        social_eng=0.0, vid_compl_rate=np.nan, conv_value=g["conversion_value"],
    )
    tt = tt.assign(
        platform="TikTok", spend=tt["cost"].astype(float),
        video_views=tt["video_views"].astype(float),
        ctr=tt["clicks"]/tt["impressions"], cpc=tt["cost"]/tt["clicks"],
        cpa=tt["cost"]/tt["conversions"],   roas=np.nan,
        social_eng=tt["likes"]+tt["shares"]+tt["comments"],
        vid_compl_rate=tt["video_watch_100"]/tt["video_views"],
        conv_value=np.nan,
    )

    cols = ["date","campaign_id","campaign_name","platform",
            "impressions","clicks","spend","conversions",
    ...
    ts = (
        ...
        .reset_index().sort_values(["date","platform"])
    )
    ts["spend_7d_avg"] = (
    ...
    tt_raw = pd.read_csv(f"{DATA_DIR}/03_tiktok_ads.csv")
    funnel = {
        "views_total": tt_raw["video_views"].sum(),
        "w25": tt_raw["video_watch_25"].sum(),
        "w50": tt_raw["video_watch_50"].sum(),
        "w75": tt_raw["video_watch_75"].sum(),
        "w100": tt_raw["video_watch_100"].sum(),
        "likes": tt_raw["likes"].sum(),
        "shares": tt_raw["shares"].sum(),
        "comments": tt_raw["comments"].sum(),
    }
    pdf_tt_funnel = pd.DataFrame([funnel])
...
def load_all(use_spark: bool = True):
    ...
    unified   — 330-row unified ad table
    ts        — daily time-series with 7-day rolling avg
...
def build_aggregates(unified: pd.DataFrame):
    ...
    plat = (
        ...
        .agg(impressions=("impressions","sum"), clicks=("clicks","sum"),
             ...
             video_views=("video_views","sum"), social_eng=("social_eng","sum"),
    ...
    google_roas = camp[camp["platform"] == "Google"][
